...
class AssociatedPressSpider(CrawlSpider):
    ...
    rules = [Rule(LinkExtractor(allow=r'\/article\/[a-zA-Z0-9\-]+'), callback='parse_item', follow=True)]
    ...
    def parse_item(self, response):
        ...
        article['body'] = response.xpath('//div[@class="Article"]/p/text()').getall()
        article['author'] = jsonData['author'][0]
        article['datePublished'] = jsonData['datePublished']
        return article
